...
void Scene::InitDefaultScene()
{
	...
	newobj = new Sphere(3.0, 7, -30.0, 2.0); //sphere 2
	newmat = new Material();
	newmat->SetAmbientColour(0.0, 0.0, 0.0);
	...
	//OBJ
	newobj = new TriangleMesh("house.obj");
	newmat = new Material();
	newmat->SetAmbientColour(0.0, 0.0, 0.0);
	newmat->SetDiffuseColour(0.5, 0.5, 0.5);
	newmat->SetSpecularColour(1.0, 1.0, 1.0);
	newmat->SetSpecPower(75);
	newobj->SetMaterial(newmat);
	m_sceneObjects.push_back(newobj);
	m_objectMaterials.push_back(newmat);
	...
	newobj = new Plane(); //an xy plane 60 units along -z axis, 
	static_cast<Plane*>(newobj)->SetPlane(Vector4D(0.0, 0.0, 1.0), -200.0);
	newmat = new Material();
	newmat->SetAmbientColour(0.0, 0.0, 0.0);
	...
	newlight->SetLightPosition(-3.0, 15.0, 5.0);
