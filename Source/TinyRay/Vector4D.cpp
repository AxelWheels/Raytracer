...
Vector4D Vector4D::Reflect (const Vector4D& n) const
{
	//return reflected ray
	return *this - ((n + n) * this->DotProduct(n));
}
	
Vector4D Vector4D::Refract(const Vector4D& n, double r_index) const
{
	double w = -this->DotProduct(n);
	double k = sqrt(1 + (w - r_index) * (w + r_index));

	//return refracted ray
	return ((*this * r_index) + (n * (w - k))).Normalise();
}
